...
/// Prefix a detached workspace's window label carries. Must match
/// `WORKSPACE_WINDOW_PREFIX` in `src-tauri/src/window.rs`, and the capability
/// entry it is granted through is the glob `workspace-*`
/// (`src-tauri/capabilities/workspace-windows.json`).
///
/// Unlike every label above this one, it is a *family*: one window per
/// workspace, labelled at runtime. Rust validates the id half before it will
/// build anything — see `workspace_window_label` — so this module never needs
/// to be the only thing standing between a bad id and a window with no
/// permissions.
export const WORKSPACE_WINDOW_PREFIX = "workspace-";

export function workspaceWindowLabel(workspaceId: string): string {
  return `${WORKSPACE_WINDOW_PREFIX}${workspaceId}`;
}

/// The workspace this window is scoped to, or `null` in every other window.
/// Read at render time by `main.tsx`, the same way the git, editor and panel
/// roots are chosen.
export function currentWorkspaceWindowId(): string | null {
  const label = currentWindowLabel();
  if (!label.startsWith(WORKSPACE_WINDOW_PREFIX)) return null;
  const id = label.slice(WORKSPACE_WINDOW_PREFIX.length);
  return id.length > 0 ? id : null;
}

export function isWorkspaceWindow(): boolean {
  return currentWorkspaceWindowId() !== null;
}
...
//
// A workspace window is a *second full workbench*, scoped to one workspace, and
// it is the one satellite that is not a consumer: it has the rail, the tabs, the
// pane tree and the terminals, and the user works in it.
//
// **Hand-off, not mirror.** A workspace is interactive in exactly one window at
// a time. `main` marks it detached, stops rendering it, and the workspace window
// owns it until it comes back. That rule is not a simplification — it is what
// makes the arrangement correct at the layer below: `PtySink` in
// `src-tauri/src/lib.rs` holds **one** channel per session, so two windows
// cannot render one terminal at the same time regardless of what the UI would
// like to do. Two subscribers means the second one silently mutes the first.
// Hand-off is the model in which that is never asked for.
//
// The consequence for storage is stated in `commands/workspaceWindows.ts`'s
// header, which is where the ownership rule lives in prose: `main` remains the
// sole writer of `localStorage`, without exception, exactly as it is for the
// editor window's tab blob.
//
// Two things cross the gap, and no more:
//
//   1. **The handoff** — which PTY sessions are live in that workspace right
//      now, so the window can *attach* to them instead of spawning shells that
//      duplicate the ones already running. This is the whole reason a channel
//      exists at all: `voidlink-terminal-tabs` deliberately persists the
//      session and not the process (see `TAB_SPECS.terminal`), so the ptyIds
//      are nowhere on disk to be read.
//   2. **"Dock me back"** — the same one-line message a panel sends, so that
//      closing the window *is* re-attaching.

/// Open the window for `workspaceId`, or focus it if it is already open.
/// Resolves to `true` when a window was actually created.
///
/// `name` is the window title only. The label is derived from the id, so
/// renaming a workspace cannot move it to a different window.
///
/// Rejects in stacked mode, where there are no satellite windows at all — the
/// caller shows the reason rather than leaving a menu row that does nothing.
export async function openWorkspaceWindow(
  ...
  if (stackedRouter) {
    throw new Error("This environment shows every surface in one window");
  }
  return invoke<boolean>("open_workspace_window", { workspaceId, name });
}

export async function closeWorkspaceWindow(workspaceId: string): Promise<void> {
  // No stacked-mode guard, for the reason `closeSidebarWindow` states: "make
  // sure that window is gone" has the same honest answer in both modes, and the
  // switch *into* stacked is precisely when there are windows to collect.
  await invoke("close_workspace_window", { workspaceId });
}

export async function isWorkspaceWindowOpen(workspaceId: string): Promise<boolean> {
  if (stackedRouter) return false;
  return invoke<boolean>("is_workspace_window_open", { workspaceId });
}

/// Bring a detached workspace's window to the front. No-op when it isn't open.
///
/// What the rail's row for a detached workspace does when clicked: the
/// workspace is not selectable in `main` while it is out, so "go to it" is the
/// only honest meaning that click has.
export async function focusWorkspaceWindow(workspaceId: string): Promise<void> {
  if (stackedRouter) return;
  await invoke("focus_workspace_window", { workspaceId });
}

const WORKSPACE_DOCK_BACK_EVENT = "voidlink://workspace-dock-back";

/// "Put this workspace back in the workbench." Emitted by a workspace window as
/// it closes, so closing *is* docking back — the same arrangement, and the same
/// awaited-emit discipline, as `requestSidebarDockBack`.
export async function requestWorkspaceDockBack(workspaceId: string): Promise<void> {
  await emitQuietly(WORKSPACE_DOCK_BACK_EVENT, workspaceId);
}

/// Subscribe to workspace dock-back requests. Workbench side.
export function onWorkspaceDockBack(
  handler: (workspaceId: string) => void,
): Promise<UnlistenFn> {
  return listenLoudly<string>(WORKSPACE_DOCK_BACK_EVENT, handler);
}

/// One live terminal, as the window taking the workspace over needs it.
///
/// `ptyId` is the field that makes this a *handoff* rather than a restore: it
/// names a process that is running right now, and the receiving window
/// subscribes to it. Everything else is what the tab needs to render — the same
/// three fields `TAB_SPECS.terminal.serialize` keeps, which is not a
/// coincidence: a handoff is that persisted shape plus the one thing disk
/// cannot carry.
export interface WorkspaceHandoffTerminal {
  id: string;
  ptyId: string;
  label: string;
  cwd: string;
...
/// The runtime state a workspace carries into its own window.
///
/// Terminals and nothing else, deliberately. Every other tab kind is a pure
/// projection of state already on disk, which the receiving window hydrates for
/// itself through the same `localStorage` read `main` does; a PTY is the one
/// thing that is a live process rather than a record, and therefore the one
/// thing that cannot be re-derived.
export interface WorkspaceHandoff {
  workspaceId: string;
  /// Keyed by worktree id, like every tab collection in the store.
  terminals: Record<string, WorkspaceHandoffTerminal[]>;
}

const WORKSPACE_HANDOFF_EVENT = "voidlink://workspace-handoff";
const WORKSPACE_HANDOFF_REQUEST_EVENT = "voidlink://workspace-handoff-request";

/// Publish a workspace's live sessions. Workbench side; safe when no workspace
/// window is open.
export async function publishWorkspaceHandoff(handoff: WorkspaceHandoff): Promise<void> {
  await emitQuietly(WORKSPACE_HANDOFF_EVENT, handoff);
}

/// Subscribe to handoffs. Workspace-window side; the handler must ignore a
/// payload for a different workspace, since this is a broadcast and a user may
/// have several windows out.
export function onWorkspaceHandoff(
  handler: (handoff: WorkspaceHandoff) => void,
): Promise<UnlistenFn> {
  return listenLoudly<WorkspaceHandoff>(WORKSPACE_HANDOFF_EVENT, handler);
}

/// Ask the workbench for the handoff. Sent by a workspace window on mount: it
/// opened *after* whatever change last triggered a broadcast, and without this
/// it would sit with an empty terminal list in front of shells that are running.
export async function requestWorkspaceHandoff(workspaceId: string): Promise<void> {
  await emitQuietly(WORKSPACE_HANDOFF_REQUEST_EVENT, workspaceId);
}

/// Subscribe to handoff requests. Workbench side.
export function onWorkspaceHandoffRequest(
  handler: (workspaceId: string) => void,
): Promise<UnlistenFn> {
  return listenLoudly<string>(WORKSPACE_HANDOFF_REQUEST_EVENT, handler);
}
