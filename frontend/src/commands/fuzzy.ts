...
/// Score-only wrapper for callers that rank rows but draw no highlight — the
/// ⇧⌘O symbol picker. `-1` for "no match", which is the contract the symbol
/// picker inherited from `FileFinder`'s original private scorer.
export function fuzzyScore(candidate: string, query: string): number {
  const match = fuzzyMatch(candidate, query, { pathAware: true });
