...
function nodeRadius(node: GraphNode): number {
  if (node.nodeType === "directory") return 6;
  if (node.nodeType === "external") return 4;
  const bytes = node.sizeBytes ?? 500;
  return Math.max(3, Math.min(12, Math.sqrt(bytes / 200)));
}
...
export function Graph2D(props: Graph2DProps) {
  ...
  onMount(() => {
    ...
    graph = ForceGraph()(containerRef)
      ...
      .linkColor((link: any) =>
        link.edgeType === "import" ? "rgba(96, 165, 250, 0.4)" : "rgba(107, 114, 128, 0.2)",
      )
      .linkWidth((link: any) => (link.edgeType === "import" ? 0.8 : 0.4))
