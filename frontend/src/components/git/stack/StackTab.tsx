  ...
  return (
    <div class="absolute inset-0 flex flex-col bg-background overflow-hidden">
      ...
      <div class="shrink-0 px-4 py-3 border-b border-border flex items-center gap-2">
        <h2 class="text-ui font-semibold flex items-center gap-1.5">
        ...
        <Show when={stack()?.needsRestack}>
          <span class="flex items-center gap-1 text-label text-warning bg-warning/10 px-2 py-0.5 rounded">
        ...
        <button
          ...
          class="flex items-center gap-1 px-2 py-1 rounded text-body border border-border text-foreground hover:bg-accent/40 disabled:text-muted-foreground/40 disabled:cursor-not-allowed disabled:hover:bg-transparent"
        >
        ...
        <button
          ...
          class="flex items-center gap-1 px-2 py-1 rounded text-body bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      <Show when={conflict()}>
        {(c) => (
          <div class="mx-4 my-3 p-3 rounded border border-destructive/40 bg-destructive/5 text-body space-y-2">
                ...
                {(p) => (
                  <li>
                    <button
                      ...
                      class="w-full flex items-center gap-2 px-2 py-1 rounded border border-warning/40 bg-warning/5 hover:bg-warning/10 text-warning text-label text-left"
                    >
            ...
            <div class="flex items-center gap-2 pt-1">
              <button
                ...
                class="text-label px-2 py-0.5 rounded border border-border text-muted-foreground hover:text-foreground hover:bg-accent/40"
              >
                Dismiss
              </button>
              <span class="text-label text-muted-foreground/70">
      ...
      <div class="flex-1 overflow-auto">
        <Show
          when={!stack.loading}
          fallback={
            <div class="flex items-center justify-center h-full text-muted-foreground text-ui">
        ...
        >
          <Show
            when={stack()}
            fallback={
              <div class="flex flex-col items-center justify-center h-full text-muted-foreground text-ui gap-2 p-6 text-center">
  ...
  return (
    <div class="flex items-stretch">
      ...
      <button
        ...
        class="font-mono text-ui font-medium hover:underline truncate min-w-0 flex-1 text-left"
        title={`Compare ${props.branch.parent}..${props.branch.name}`}
      >
        {props.branch.name}
      </button>
      <Show when={props.branch.aheadOfParent > 0}>
        <span class="text-success text-label tabular-nums">
          ↑{props.branch.aheadOfParent}
        </span>
      </Show>
      <Show when={drift()}>
        <span
          class="flex items-center gap-0.5 text-label text-warning"
      ...
      <Show when={props.branch.prNumber}>
        {(n) => (
          <span class="flex items-center gap-0.5 text-label text-muted-foreground tabular-nums">
      ...
      <button
        ...
        class="flex items-center gap-1 text-label px-2 py-0.5 rounded border border-border text-muted-foreground hover:text-foreground hover:bg-accent/40 transition-colors"
      ...
      <button
        ...
        class="text-label px-2 py-0.5 rounded border border-border text-foreground hover:bg-accent/40 disabled:text-muted-foreground/40 disabled:cursor-not-allowed disabled:hover:bg-transparent"
      >
        Restack
      </button>
      <div class="relative">
        ...
        <Show when={menuOpen()}>
          <div class="absolute right-0 top-full mt-1 w-44 rounded-md border border-border bg-popover shadow-lg z-50 py-1 text-body">
            <button
              onClick={() => {
                setMenuOpen(false);
                props.onCopy();
...
function SubmitResultsModal(props: { results: SubmitResult[]; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <h3 class="text-ui font-semibold mb-2 flex items-center gap-1.5">
        ...
        <ul class="space-y-1.5">
          <For each={props.results}>
            {(r) => (
              <li class="flex items-center gap-2 text-body border border-border rounded px-2 py-1.5">
                <span class="font-mono text-foreground/85 truncate flex-1" title={r.branch}>
                  {r.branch}
                </span>
                <Show when={r.outcome.kind === "created"}>
                  <span class="text-success text-label">created</span>
                </Show>
                <Show when={r.outcome.kind === "updated"}>
                  <span class="text-info text-label">updated base</span>
                </Show>
                <Show when={r.outcome.kind === "noChange"}>
                  <span class="text-muted-foreground text-label">no change</span>
                </Show>
                <Show when={r.outcome.kind === "failed"}>
                  {(() => {
                    const failed = r.outcome as Extract<typeof r.outcome, { kind: "failed" }>;
                    return (
                      <span class="text-destructive text-label" title={failed.reason}>
                        failed
                      </span>
                    );
                  })()}
                  ...
                  {(() => {
                    ...
                    return (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        class="text-primary text-label tabular-nums hover:underline"
                      >
                        #{ok.number}
                      </a>
                    );
        ...
        <Show when={props.results.some((r) => r.outcome.kind === "failed")}>
          <div class="mt-3 text-label text-muted-foreground">
        ...
        <div class="mt-3 flex justify-end">
          <button
            onClick={props.onClose}
            class="text-body px-3 py-1 rounded border border-border text-muted-foreground hover:text-foreground hover:bg-accent/40"
          >
            Close
          </button>
        </div>
...
function TrunkRow(props: { trunk: string }) {
  return (
    <div class="flex items-center gap-2 px-3 py-2 text-muted-foreground text-body">
      <span class="w-3 text-center shrink-0 font-mono">└</span>
      <span class="font-mono">{props.trunk}</span>
      <span class="text-micro tracking-wide opacity-70">Trunk</span>
    </div>
  );
}
