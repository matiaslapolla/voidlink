  ...
  return (
    <div class="absolute inset-0 flex flex-col bg-background">
      <div class="flex items-center gap-2 px-3 py-2 border-b border-border shrink-0">
        ...
        <span class="text-label text-muted-foreground tabular-nums">
        ...
        <button
          ...
          class="flex items-center gap-1 px-2 py-1 rounded text-ui bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed"
        >
        ...
        >
          <div class="flex-1 overflow-y-auto scrollbar-thin p-3 space-y-3">
            ...
            <details class="border border-border/50 rounded">
              <summary class="px-3 py-2 text-body text-muted-foreground cursor-pointer hover:bg-accent/30">
...
function ConflictBlockCard(props: {
  ...
  return (
    <div class="border border-border rounded-md overflow-hidden">
      <div class="flex items-center gap-2 px-3 py-1.5 bg-muted/40 border-b border-border text-label text-muted-foreground">
        ...
        <button
          ...
          class="px-2 py-0.5 rounded bg-info/15 text-info hover:bg-info/25 text-label"
        >
          Accept ours
        </button>
        <button
          onClick={props.onAcceptTheirs}
          class="px-2 py-0.5 rounded bg-warning/15 text-warning hover:bg-warning/25 text-label"
        >
          Accept theirs
        </button>
        <button
          onClick={props.onAcceptBoth}
          class="px-2 py-0.5 rounded bg-primary/15 text-primary hover:bg-primary/25 text-label"
        >
          Accept both
        </button>
      </div>
...
function CodeBlock(props: { label: string; tone: "info" | "warning" | "muted"; text: string }) {
  ...
  return (
    <div class="flex flex-col">
      <div class={`px-3 py-1 text-micro tracking-wide ${toneClass()}`}>{props.label}</div>
      <pre class="px-3 pb-2 text-body font-mono whitespace-pre-wrap break-words overflow-x-auto">
        {props.text || "(empty)"}
      </pre>
    </div>
  );
...
function RawEditor(props: { buffer: string; onEdit: (v: string) => void }) {
  return (
    <textarea
      ...
      spellcheck={false}
      class="w-full min-h-[200px] px-3 py-2 font-mono text-body bg-muted/30 border-t border-border outline-none resize-y"
    />
  );
}
...
function ResolvedPane(props: { buffer: string; onEdit: (v: string) => void; onSave: () => void }) {
  return (
    <div class="flex-1 flex flex-col">
      <div class="px-3 py-2 text-body text-success border-b border-border bg-success/5 flex items-center gap-2">
      ...
      <textarea
        ...
        onInput={(e) => props.onEdit(e.currentTarget.value)}
        spellcheck={false}
        class="flex-1 px-3 py-2 font-mono text-body bg-background outline-none resize-none"
      />
    </div>
  );
}
