...
pub fn make_event(level: &str, message: &str) -> AgentEvent {
    AgentEvent {
            ...
            "delete" => {
                if abs_path.exists() {
                    if let Err(e) = std::fs::remove_file(&abs_path) {
                        ...
                        emit_event!("diff-update", file_path);
                    }
                }
            }
            _ => {
                if let Some(content) = change["content"].as_str() {
                    ...
                    if let Err(e) = std::fs::write(&abs_path, content) {
                        ...
                        // Signal consumers (e.g. live-diff UI) that the worktree
                        // has changed on disk. A separate event level so it's
                        // easy to filter out of the user-visible chat stream.
                        emit_event!("diff-update", file_path);
                    }
                }
            }
...
pub fn list_files_brief(path: &str, max: usize) -> String {
