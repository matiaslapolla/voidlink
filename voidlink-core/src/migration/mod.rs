...
pub struct MigrationState {
    pub db: SqliteStore,
    pub scan_jobs: Arc<Mutex<HashMap<String, ScanProgress>>>,
    pub run_cache: Arc<Mutex<HashMap<String, RunState>>>,
    pub provider: Arc<Mutex<Arc<ProviderAdapter>>>,
...
impl MigrationState {
    ...
    pub fn get_provider(&self) -> Arc<ProviderAdapter> {
        self.provider
            .lock()
            .map(|g| g.clone())
...
pub fn reload_provider(state: &MigrationState) -> Result<(), String> {
    let new_provider = Arc::new(ProviderAdapter::new());
    *state.provider.lock().map_err(|e| e.to_string())? = new_provider;
    Ok(())
}

pub fn update_scan<F>(state: &MigrationState, job_id: &str, updater: F) -> Result<(), String>
where
...
pub fn search_repository(
    ...
    perform_search(state, query, options)
}
...
pub fn get_run_status(
    ...
    let run = state.db.load_run_state(run_id)?;
    {
        ...
        cache.insert(run_id.to_string(), run.clone());
    }
    Ok(run)
}

pub fn get_startup_repo_path(state: &MigrationState) -> Option<String> {
