...
#[tauri::command]
pub(crate) fn load_api_key(provider: String) -> Result<Option<String>, String> {
    let entry = keyring::Entry::new(KEYRING_SERVICE, &format!("provider_{provider}"))
        .map_err(|e| e.to_string())?;
    match entry.get_password() {
        Ok(key) => Ok(Some(key)),
        Err(keyring::Error::NoEntry) => Ok(None),
        // Some backends report missing entries as other error variants
        Err(_) => Ok(None),
    }
}

#[tauri::command]
pub(crate) fn save_provider_settings(settings: ProviderSettings) -> Result<(), String> {
...
#[tauri::command]
pub(crate) fn load_provider_settings() -> Result<ProviderSettings, String> {
    let path = settings_path()?;
    if !path.exists() {
