...
impl SqliteStore {
    ...
    pub(crate) fn repo_id_for_path(&self, root_path: &str) -> Result<Option<String>, String> {
        let conn = self.open()?;
        conn.query_row(
            "SELECT id FROM repos WHERE root_path = ?1",
            params![root_path],
    ...
    pub(crate) fn load_workflow(&self, workflow_id: &str) -> Result<WorkflowDsl, String> {
        let conn = self.open()?;
        let raw = conn
            .query_row(
    ...
    pub(crate) fn create_run(&self, run_id: &str, workflow: &WorkflowDsl) -> Result<(), String> {
        let conn = self.open()?;
    ...
    pub(crate) fn load_run_state(&self, run_id: &str) -> Result<RunState, String> {
        let conn = self.open()?;
        let run_row = conn
            .query_row(
