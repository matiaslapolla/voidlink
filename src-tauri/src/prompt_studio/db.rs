...
impl PromptStore {
    ...
    pub(crate) fn list_prompts(&self) -> Result<Vec<PromptSummary>, String> {
        let conn = self.open()?;
        let mut stmt = conn
            .prepare(
    ...
    pub(crate) fn get_prompt(&self, id: &str) -> Result<PromptFull, String> {
        let conn = self.open()?;
        let row = conn
            .query_row(
    ...
    pub(crate) fn save_prompt(&self, input: &SavePromptInput) -> Result<PromptFull, String> {
        let conn = self.open()?;
        let now = now_ms();
    ...
    pub(crate) fn delete_prompt(&self, id: &str) -> Result<(), String> {
        let conn = self.open()?;
    ...
    pub(crate) fn toggle_favorite(&self, id: &str) -> Result<bool, String> {
        let conn = self.open()?;
    ...
    pub(crate) fn list_tags(&self) -> Result<Vec<PromptTag>, String> {
        let conn = self.open()?;
        let mut stmt = conn
            .prepare(
    ...
    pub(crate) fn get_versions(&self, prompt_id: &str) -> Result<Vec<PromptVersion>, String> {
        let conn = self.open()?;
        let mut stmt = conn
            .prepare(
    ...
    pub(crate) fn save_execution(&self, exec: &PromptExecution) -> Result<(), String> {
        let conn = self.open()?;
        conn.execute(
    ...
    pub(crate) fn rate_execution(&self, execution_id: &str, rating: i32) -> Result<(), String> {
        let conn = self.open()?;
        conn.execute(
