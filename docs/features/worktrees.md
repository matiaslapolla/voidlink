...
3. The directory path is **computed, not chosen**: the repo path with a
   slugified branch suffix. A branch `feature/x` in `/repos/app` produces
   `/repos/app-feature-x`.
...
None. There is no palette action and no keybinding for any worktree operation.

## Gotchas and limits

- **You cannot choose the directory.** There is no path picker. Two branches
  whose names slugify to the same string collide, and the resulting
  `path already exists: <path>` is unrecoverable from the UI.
