...
| **`failed` from a terminal command** | Still unreachable, and it is the oldest hole in the signal vocabulary. `pty-exit` carries the exit status, so a shell that *dies* can be reported; a command that fails *inside* a live shell cannot, because `terminalWatch` infers completion from the process poll going idle and idle carries no status. Closing it needs shell integration — OSC 133 or a wrapper — which is a different piece of work from watching a PID and has never been costed. |
