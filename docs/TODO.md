...
| Item | State |
|---|---|
| **The coverage backlog** | **Done.** `GitSidebar` is mounted — 31 tests against a real `createAppStore({ persist: false })` and the Tauri stub, covering all three the audit named: an `MM` file is two rows with opposite controls and one cursor between them, a refresh keeps its rows through the refetch, a failing section renders the boundary and loses only itself. `ChangedFileTree` (15), `OperationBanner` (14), `ViewSwitcher`, `Splitter` (14) and hunk actions (9) alongside it. |
| **The geometry rows of that backlog** | **Done bar one.** `SplitDiffRenderer` (8: the hover/focus reveal of the hunk toolbar, sticky headers through a real scroll, split-column alignment under a 2000-character line), the splitter (11: the 8px hit area, no reflow on hover, the `calc(var(--island-gap)/2 - 4px)` gap offset) and the file tree's virtualized list (6) are in the `browser` project, on top of the commit graph and `TabStrip`. Five files, 32 tests, ~11s wall clock against ~7s for the original two. **Still open: the MRU overlay**, which lives inside `MainSurface` and needs that surface mounted rather than a component extracted from it. **Correction to this row as written:** `SplitDiffRenderer` is not virtualized — it has no `createVirtualizer` — so the "virtualized rows past the viewport" line never applied to it. The four windowed lists are `FileTree`, `CommitGraph`, `GitSidebar`'s `VirtualFileList` and `BrainSurface`; the first two are covered and the third takes the unwindowed branch below 40 rows. |
